...
inline wchar_t* _Maklocwcs(const wchar_t* _Ptr) { // copy NTWCS to allocated storage
    ...
    wchar_t* _Ptrdest = (wchar_t*) _calloc_dbg(_Count, sizeof(wchar_t), _CRT_BLOCK, __FILE__, __LINE__);
...
public:
    ...
    // @CL 260679: commented out _CRTIMP2_PURE_IMPORT - see XLOCNUM_HACK_TO_FIX_NUM_PUNCT_IN_OUR_UTF8_STREAM                                                                                                  
...
protected:
    virtual __CLR_OR_THIS_CALL ~numpunct() noexcept { // destroy the object
        _Tidy();
    }
    ...
    virtual _Elem __CLR_OR_THIS_CALL do_decimal_point() const { // return decimal point
        return _Dp;
    }

    virtual _Elem __CLR_OR_THIS_CALL do_thousands_sep() const { // return thousands separator
        return _Kseparator;
    }

    virtual string __CLR_OR_THIS_CALL do_grouping() const { // return grouping string
        return string(_Grouping);
    }

    virtual string_type __CLR_OR_THIS_CALL do_falsename() const { // return name for false
        return string_type(_Falsename);
    }

    virtual string_type __CLR_OR_THIS_CALL do_truename() const { // return name for true
...
protected:
    virtual __CLR_OR_THIS_CALL ~numpunct_byname() noexcept { // destroy the object
    }
};
...
protected:
    virtual __CLR_OR_THIS_CALL ~num_get() noexcept { // destroy the object
    }
...
protected:
    virtual __CLR_OR_THIS_CALL ~num_put() noexcept { // destroy the object
    }
...
private:
    ...
    _OutIt __CLRCALL_OR_CDECL _Fput(_OutIt _Dest, ios_base& _Iosbase, _Elem _Fill, const char* _Buf,
        ...
        const char* _Pg = &_Grouping[0];
        while (*_Pg != CHAR_MAX && '\0' < *_Pg && (size_t) *_Pg < _Off - _Prefix) { // add thousands separator
    ...
    _OutIt __CLRCALL_OR_CDECL _Iput(_OutIt _Dest, ios_base& _Iosbase, _Elem _Fill, char* _Buf,
        ...
        if (*_Pg != CHAR_MAX && '\0' < *_Pg) { // grouping specified, add thousands separators
            const _Elem _Kseparator = _Punct_fac.thousands_sep();
            while (*_Pg != CHAR_MAX && '\0' < *_Pg && (size_t) *_Pg < _Count - _Prefix) { // insert thousands separator
                _Count -= *_Pg;
