...
//   ConvertPSResourcesToPMTags
//   DESCRIPTION:
//   Convert AdobePSL::path into a standard TIFF clippath.
//   imageWidth and imageHeight are the PSL image’s width and height
//   IImageAttributes are populated which are required to draw the TIFF clippath.
bool16 PUBLIC_DECL ConvertPSResourcesToPMTags (const AdobePSL::Path *clipPath,
                                uint32    imageWidth,
                                uint32    imageHeight,
                                IImageAttributes* iImageAttributes);
