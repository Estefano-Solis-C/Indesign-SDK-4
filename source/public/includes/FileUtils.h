...
#include "LocaleSetting.h"
	...
	enum CFURLPathStyle { 	kCFURLPOSIXPathStyle = 0, kCFURLHFSPathStyle, kCFURLWindowsPathStyle };
#endif
...
public:
	...
	*/
	...
	typedef enum { kSystemPath, kWindowsPath } DirPath;
	static HMODULE LoadLibraryUsingAbsolutePath(const std::wstring& componentName, DirPath dirPath = kSystemPath);
#endif

	 /**
     Check if file is accessible for saving. 
     @param  destFile    IN IDFile destination file path
     @return kNoError if the destFile is accesible, kAccessDenied if it is not.
     */
	...
	*/
	...
	@param  originalPath	IN IDFile object to append paths
	@param  pathsToAppend	IN PMString paths to be appended to IDFile in the given order
	*/
	...
	Appends a vector of paths to IDFile object
	@param  originalPath	IN IDFile object to append paths
	@param  pathsToAppend	IN wchar_t* paths to be appended to IDFile in the given order
	*/
    static void AppendPaths(IDFile *originalPath, const std::vector<const wchar_t*> &pathsToAppend);

	// HFS directory seperator

	/**
	Internal use only
	Returns the HFS-Style directory separator in type char
	@return HFS Path-Style directory separator
	*/
	static char   GetHFSDirectorySeparator();

	/**
	Internal use only
	Returns the HFS-Style directory separator in type textChar
	@return HFS Path-Style directory separator
	*/
	static textchar GetHFSDirectorySeparatorTextChar();

	/**
	Internal use only
	Returns the HFS-Style directory separator in type PMString
	@return HFS Path-Style directory separator
	*/
	static PMString GetHFSDirectorySeparatorPMString();

	// Posix directory seperator

	/**
	Internal use only
	Returns the Posix-Style directory separator in type char
	@return Posix Path-Style directory separator
	*/
	static char   GetPosixDirectorySeparator();

	/**
	Internal use only
	Returns the Posix-Style directory separator in type textChar
	@return Posix Path-Style directory separator
	*/
	...
	/**
	Internal use only
	Returns the Posix-Style directory separator in type PMString
	@return Posix Path-Style directory separator
	*/
...
#ifdef MACINTOSH
	// HFS <-> Posix conversion API's

	/**
	Converts HFS style path to Posix style path
	@param  hfsPath		IN  hfs style path to convert to posix style path
	@param  posixPath	OUT Resulting posix style path
	*/
	static void HFSPathToPOSIXPath(const PMString &hfsPath, PMString &posixPath);
	...
	@param  posixPath	OUT Resulting posix style path
	*/
	static void HFSPathToPOSIXPath(const std::string &hfsPath, std::string &posixPath);

	/**
	Converts Posix style path to HFS style path
	@param  posixPath	IN  posix style path to convert to hfs style path
	@param  hfsPath		OUT Resulting hfs style path
	*/
	static void POSIXPathToHFSPath(const PMString &posixPath, PMString &hfsPath);
	...
	@param  hfsPath		OUT Resulting hfs style path
	*/
	static void POSIXPathToHFSPath(const std::string &posixPath, std::string &hfsPath);

	/**
	Appends a vector of paths to PMString object
	@param  originalPath	IN PMString object to append paths
	@param  pathsToAppend	IN PMString paths to be appended to IDFile in the given order
	*/
	static  void AppendPaths(PMString *originalPath, const std::vector<const PMString> &pathsToAppend);
	...
	@param  pathsToAppend	IN wchar_t* paths to be appended to IDFile in the given order
	*/
	static  void AppendPaths(PMString *originalPath, const std::vector<const wchar_t*> &pathsToAppend);
