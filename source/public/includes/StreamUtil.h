...
#define ID_FORWARD_DECLARE(_X) \
                    class _X; \
                    using _X##Ptr = std::shared_ptr<_X>;\
                    using Const##_X##Ptr = std::shared_ptr<const _X>;\
                    using _X##WeakPtr = std::weak_ptr<_X>;\
                    using Const##_X##WeakPtr = std::weak_ptr<const _X>;\
                    using _X##UniquePtr = std::unique_ptr<_X>;\
...
class PUBLIC_DECL StreamUtil {
    ...
    /*
    Internal Use Only.
    Used to copy data from source db to destination database.
    @param source source database
    @param sourceUID the UID from source database
    @param target target database
    @param targetUID the UID from target database
    */
	
    static IPMStream* CreateCustomDBStreamWrite(IDataBase* source, UID sourceUID, IDataBase* target, UID targetUID);
    /*
    	Internal Use Only.
	Wrapper stream for composite type files. Cannot actually perform read write directly. 
	Can be used to access the composite pointer.
    */
