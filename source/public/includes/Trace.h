...
#ifdef MACINTOSH
    #ifndef DEBUG_LAYER_LOG
    // If we want to enable logs in console then enable below if.
        #if 0
            #define DEBUG_LAYER_LOG(category, ...) NSLog((@""), ##__VA_ARGS__)
        #else
            #define DEBUG_LAYER_LOG(category, ...) TRACE_INTERNAL(kTrue, category, __FILE__, __LINE__, __VA_ARGS__ )
        #endif
    #endif
#else
// We don't want layer logs for windows.
    #ifndef DEBUG_LAYER_LOG
        #define DEBUG_LAYER_LOG(...)              ((void) 0)
    #endif
#endif
