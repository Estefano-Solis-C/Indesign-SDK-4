...
class IEnvironment : public IPMUnknown
{
	...
	*/
	...
	Get the id for the Custom URL  open files message. Client code should not call this. It's used internally to handle passing a document from one instance to another.
		@return an id associated with opening a file.
	*/
		virtual const uint32 GetCustomOpenFilesMessageId() const = 0;
	#endif
