...
public:
	...
	/** The ImportImageCmd requires the client to pass in a valid IDataBase, UID, and stream.
		...
		@param transparencyAlphaIndex - IN: If >= 0, it is the index of the alpha to use as
			a transparency mask.
        @param allowAutoEmbedding - IN: Pass kFalse to disable auto embedding during place.
	*/
	virtual void Set(IDataBase* db, UID uid, IPMStream *stream,
		...
		int16 transparencyAlphaIndex = -1,	// If >= 0, it is the index of the alpha to use as
											// a transparency mask.
		bool8 allowAutoEmbedding = kTrue) = 0;
	...
	*/
    ...
    /** @return kFalse if placed file must not be auto embedded. Auto embedding happens when file size < 48k
    */
    virtual bool8 GetAllowAutoEmbedding() const = 0;

    /** @return classId of IImageReadFormat using which the image should be placed. 
    */
    virtual ClassID GetFormatClassId() const = 0;
		...
		@param transparencyAlphaIndex - IN: If >= 0, it is the index of the alpha to use as
			a transparency mask.
        @param allowAutoEmbedding - IN: Pass kFalse to disable auto embedding during place.
		@param formatClassId - IN: Pass the classId of IImageReadFormat using which the imaged should be placed.
	*/
	virtual void Set(IDataBase* db, UID uid, IPMStream *stream,
		bool8 showProgressBar = kTrue,
		...
		ClassID formatClassId = kInvalidClass) = 0;
};
