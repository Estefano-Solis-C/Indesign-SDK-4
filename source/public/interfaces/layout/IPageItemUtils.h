...
public:
	...
	*/
	...
	/** Finds if the selected Graphic frame has some empty space.
		@param itemUIDRef [IN] the UIDRef of the selected Graphic frame.
		@return A bool.
	*/
	virtual bool16 HasFrameEmptySpace(UIDRef itemUIDRef, bool16 considerStroke = kTrue) const = 0;

	/** Finds if for the selected Graphic frame we will be able to expand the original image.
		@param itemUIDRef [IN] the UIDRef of the selected Graphic frame.
		@return A bool.
	*/
	virtual bool16 IsGenExpandApplicable(UIDRef itemUIDRef) const = 0;
};
