...
class IFindChangeFormatCmdData : public IPMUnknown
{
	public:
		...
		/**
			Gets change case option.

			@return PMString&    Change case option string.
		 */
		virtual const PMString& GetChangeCaseOption() = 0;
		/**
			Sets change Change Case Option.

			@param changeCaseOption    Change case option string.
		 */
		virtual void SetChangeCaseOption(const PMString& changeCaseOption) = 0;
		/**
			turns on clear change case option flag.
		 */
		virtual void ClearChangeCaseOption() = 0;
		/**
			Checks if change case option is set or not.

			@return bool16    kTrue if change case option is set.
		 */
		virtual bool16 IsChangeCaseOptionSet() = 0;
		/**
			Checks if clear changecase option flag is on or off.

			@return bool16    kTrue if clear change case option flag is on.
		 */
		virtual bool16 IsChangeCaseOptionCleared() = 0;
