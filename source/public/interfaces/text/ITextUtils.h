...
public:
	...
	***Client responsibility to clear the design axes after API use***
	...
	@param pointSize - default -1, but for the cases where we want optical axis mapping we need to pass it.
	*/
	virtual bool16 RemapFontStylesWithOPSZ(IDataBase* dataBase, const AttributeBossList& stateOverrides, const AttributeBossList& newOverrides,
		PMString* styleName, Fixed*& axesInfo, int& numAxes, ClassID textAttrFontUIDBoss = kTextAttrFontUIDBoss, ClassID textAttrFontStyleBoss = kTextAttrFontStyleBoss,
		bool16 remapExactFontStylesOnly = kFalse, PMReal pointSize = -1) = 0;
};
