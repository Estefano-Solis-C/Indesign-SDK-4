	...
	*/
	...
	public:
		...
		*/
		...
		/** Description: GetModifyIndentsAroundTextWrap() returns kTrue if Text Indents would be honoured along
			with Indents of Text Wrap. This modifies the Text Flow for Bullets and Numbering. kFalse if Text Indents
			and Text wrap indents are to be considered separately.
			@param workspace IN - the UIDRef of the target workspace.  This UIDRef can be either a
			document workspace or the session workspace.
			@param modifyIndent OUT - kTrue if Text Indents would be honoured along with Indents of Text Wrap.
			@return ErrorCode - kSuccess if successful, an error otherwise
		*/
		virtual ErrorCode GetModifyIndentsAroundTextWrap(const UIDRef& workspace, bool16& modifyIndent) const = 0;
        
		/** Description: SetModifyIndentsAroundTextWrap() is used to set Indentation behaviour where in Text Indents
			would be honoured along with Indents of Text Wrap. This modifies the Text Flow for Bullets and Numbering.
			@param workspace IN - the UIDRef of the target workspace.  This UIDRef can be either a
			document workspace or the session workspace.  Changing the session workspace setting
			will cause all new documents to take on the new setting.  Changing the document workspace
			setting changes the setting in only that document.
			@param modifyIndent IN - pass kTrue to make Text Indents honour Text Wrap Indents and
			modify Text Flow for Bullets and Numbering. Pass kFalse to keep the old behaviour. Due to this
			the Bullets and Numbering layout around the Text Wrap behaves same as that in normal paragraph
			@return ErrorCode - kSuccess if successful, an error otherwise
		*/
