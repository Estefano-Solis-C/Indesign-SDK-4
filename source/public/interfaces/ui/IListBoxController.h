...
public:
	...
	*/
	...
	/** Fills in collection with indices of elements from the list with checkbox value ticked.
		Checked item list is different from the selected items
		The elements are in the order the user checked items.
		@param checkedItems OUT parameter, holds indices of cheched items
	*/		
	virtual void				GetCheckedItems(K2Vector<int32>& checkedItems) const = 0;

	/** Sets the checkbox values of the items with indexes matching from vecIndices
		Do it only, when the dropdown list has checkboxes in it (would be the case if multiple selection with checkboxes enabled)
		The elements are in the order the user checked items.
		@param vecIndices list of items of the dropdown, whose checkbox values needs to be ticked
		@param invalidate whether control should be invalidated
		@param bNotify specify whether registered observers should be notified of this change
	*/
