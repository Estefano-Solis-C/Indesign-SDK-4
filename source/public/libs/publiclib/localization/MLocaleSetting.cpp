...
PMLocaleId MLocaleSetting::scriptLookup(int script)
{
	...
	// Removed smRoman, smJapanese, smSimpChinese, smTradChinese, smKorean as they have been deprecated.
	// Replaced those deprecated enum by CTtypes (CTScript enum).
	int32 app = GetLocale().GetProductFS() << 16;	// app code in high word
	switch (script)
    {
		case kCTRomanScript:				return PMLocaleId(app | kRomanLanguageFS, k_enUS);
		case kCTJapaneseScript:				return PMLocaleId(app | kJapaneseLanguageFS, k_jaJP);
		case kCTSimplifiedChineseScript: 	return PMLocaleId(app | kJapaneseLanguageFS, k_zhCN);
		case kCTTraditionalChineseScript:	return PMLocaleId(app | kJapaneseLanguageFS, k_zhTW);
		case kCTKoreanScript:				return PMLocaleId(app | kJapaneseLanguageFS, k_koKR);
...
int16 MLocaleSetting::GetSystemScript()
{
	if( ts_gLocaleSetting.fSystemScript == kCTDontKnowScript )
	{
		...
		// Removing `TextEncoding`, `ScriptCode` and `RevertTextEncodingToScriptInfo` as they were part of the carbon framework.
		// Replacing `TextEncoding` to `CFStringEncoding` is a part of the Core Foundation framework in macOS
		// There's no direct api replacement for `RevertTextEncodingToScriptInfo`, which converts system encoding to corresponsding script.
		// Doing manual mapping of encoding to CTTypes (CTScript enum).
...
// #endif
		...
		switch(encoding)
		{
...
#ifdef qME
		        ts_gLocaleSetting.fSystemScript = kWRTurkishScript;
#else
				ts_gLocaleSetting.fSystemScript = kCTRomanScript;
#endif
				break;
			...
			case kCFStringEncodingMacArabic:
				...
				break;
#ifdef qME
		        ts_gLocaleSetting.fSystemScript = kWRBalticScript;
#else
				ts_gLocaleSetting.fSystemScript = kCTCyrillicScript;
#endif
				break;
			...
			case kCFStringEncodingMacVietnamese:
				ts_gLocaleSetting.fSystemScript = kCTVietnameseScript;
				break;
			case kCFStringEncodingMacJapanese:
				ts_gLocaleSetting.fSystemScript = kCTJapaneseScript;
				break;
			case kCFStringEncodingMacChineseSimp:
				ts_gLocaleSetting.fSystemScript = kCTSimplifiedChineseScript;
				break;
			case kCFStringEncodingMacKorean:
				ts_gLocaleSetting.fSystemScript = kCTKoreanScript;
				break;
			case kCFStringEncodingMacChineseTrad:
				ts_gLocaleSetting.fSystemScript = kCTTraditionalChineseScript;
				break;

			default:
				// Default case: handle unknown encoding
				// Log an assertion message and set a fallback script type
...
bool16  MLocaleSetting::GetPreferenceFileName(const PMLocaleId& locale, PMString& outStrPrefFileName)
{
    ...
    if( appFolderObtained == kTrue )
    {
        ...
        strPresetsThemePath.Append(FileUtils::GetPosixDirectorySeparatorPMString());
