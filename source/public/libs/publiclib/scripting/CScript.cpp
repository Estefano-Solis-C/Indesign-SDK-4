...
CScript::~CScript()
{
	...
	{
		std::for_each( fClientObjects.begin(), fClientObjects.end(), std::mem_fn( &ScriptClientObject::ReleaseRefFromObjectModel ) ) ;
...
void CScript::NotifyClientObjectsToReleaseMe()
{
	if ( !fClientObjects.empty() )
	{
		...
		std::for_each( temp.begin(), temp.end(), std::mem_fn( &ScriptClientObject::AddRefFromObjectModel ) ) ;
		std::for_each( temp.begin(), temp.end(), boost::bind( &ScriptClientObject::ReleaseReferencesToObjectModel, _1, this ) ) ;
		std::for_each( temp.begin(), temp.end(), std::mem_fn( &ScriptClientObject::ReleaseRefFromObjectModel ) ) ;
	}
}
